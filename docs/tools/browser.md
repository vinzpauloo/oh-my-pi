...
4. `open` acquires a browser handle through `acquireBrowser()` (`packages/coding-agent/src/tools/browser/registry.ts`):
   ...
   - `spawned` first tries `findReusableCdp()`; if another non-CDP process already uses that executable, the call fails closed instead of terminating it; otherwise it allocates a free loopback port, spawns the executable with `--remote-debugging-port=<port>`, waits for CDP, then connects;
...
- **Browser kind**
  ...
  - **Spawned app (`app.path`)**: reuses an existing CDP-enabled process for that executable when possible. If another non-CDP process already uses the executable, the call fails closed and directs the caller to `app.cdp_url`, `app.relay`, or an isolated executable such as Chrome for Testing. Only a newly spawned child is owned and eligible for later termination. No stealth patches are injected.
